...
- **Hash skip**: `syncEngine.ts` compares a content hash to avoid re-pushing unchanged notes. After the multi-instance migration `confluence_last_hash` is keyed by `instanceId → pageId → hash` (two-level nested Record). The instanceId layer is required because Confluence pageIds are local to an installation — two Server/DC instances can have the same pageId, and a single-level key would let engine A's stamp vouch for engine B's target. The engine passes only its own slice delta to writeBinding; the atomic merge inside writeBinding (under a plugin-wide mutex) preserves foreign slices verbatim. Production code only handles the current shape — pre-multi-instance string-form `lastHash` is migrated to per-instance form by `migrateLegacyFrontmatter` on plugin load.
- **First-sync-after-upgrade caveat**: `migrateLegacyFrontmatter` and `migrateLegacyUsernames` call `processFrontMatter` on each note, which triggers `metadataCache.on('changed')` asynchronously. The very next sync (e.g., from `syncOnStartup` 5s after onload) may run before the cache is refreshed, so it reads the legacy shape. For `migrateLegacyFrontmatter` this means a one-time no-op content re-push (not data loss); for `migrateLegacyUsernames` it means the mention resolver sees the legacy string form, returns null, and the mention degrades to plain `@Name` on the first sync only. Subsequent syncs resolve correctly.
- **Attachments**: `confluence_attachments` frontmatter caches `instanceId → pageId → filename → {hash, id}` to skip re-uploads. The instanceId layer prevents cross-instance collisions; the pageId layer isolates multi-target uploads. Pre-multi-instance flat shape `{ filename: { hash, id } }` is migrated to per-instance form by `migrateLegacyFrontmatter` on plugin load.
- **Mentions**: `confluence_username` frontmatter (on person notes referenced by `@[[Name]]`) holds a `PerInstanceUsernameMap` keyed by `ConfluenceInstance.id`. Each engine reads only its own slice; missing slice → resolver returns null → markdownConverter degrades to plain `@Name` on that instance only. Pre-multi-instance flat-string shape is migrated to the per-instance map by `migrateLegacyUsernames` inside the same gated migration block as `migrateLegacyFrontmatter`. Cloud mentions remain unsupported (still requires `ri:account-id`).
