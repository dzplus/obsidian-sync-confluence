...
- **First-sync-after-upgrade caveat**: `migrateLegacyFrontmatter` calls `processFrontMatter` on each note, which triggers `metadataCache.on('changed')` asynchronously. The very next sync (e.g., from `syncOnStartup` 5s after onload) may run before the cache is refreshed, so it reads the legacy shape and pushes content once. This is a one-time no-op re-push (not data loss) and only affects users with `syncOnStartup` enabled. Subsequent syncs skip correctly.
